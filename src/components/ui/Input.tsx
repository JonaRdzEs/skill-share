    ...
    <div className={`flex flex-col justify-start items-start gap-2 ${className}`}>
      ...
      <div className="border border-gray-300 rounded-lg relative w-full h-full">
        ...
        <input
          id={id}
          {...props}
          className={`w-full h-full ${leftIcon ? "pl-12" : "p-5"} pr-5 py-3 rounded-lg disabled:bg-background disabled:text-secondary-txt focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent ${error && "ring ring-red-500"}`}
        />
        {rightIcon && (
